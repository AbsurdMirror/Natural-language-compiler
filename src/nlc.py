...
parser.add_argument("--model_speed", "-m", help="AI模型速度，默认为'auto'", default='auto')

# 添加可选参数：输出文件路径
parser.add_argument("--output_file_path", "-o", help="输出文件的路径，默认为'output.txt'", default='')

# 添加可选参数：debug模式
parser.add_argument("--debug", "-d", help="是否启用调试模式，默认为False", action="store_true")
...
if args.ai_model == 'Baidu':
  ...
  if not resp_data:
    print_error(' Failed to get response from Baidu API.')
    exit(1)
    ...
    output_file_path = args.output_file_path if args.output_file_path else default_output_file_path
